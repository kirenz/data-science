...
-   "Meet the toolkit: Programming" and
...
We've already discussed a few new terms, so let's define them before we proceed.
...
-   **Repository:** A Git repository contains all of your project's files and stores each file's revision history.
    ...
    -   All repos associated with this course are housed in the course GitHub organization. The organization is set up such that students can only see repos they have access to.
...
-   have an Account in RStudio Cloud
...
The first step is always to bring these files into your local machine so that you can edit them, run them, view your results, and interpret them.
...
The next few steps will walk you through the process of getting information of the repo to be cloned, cloning your repo in a RStudio Cloud project, and getting started with the analysis.
...
Go to [rstudio.cloud](https://rstudio.cloud/ "RStudio Cloud") and then **navigate to your workspace** via the left sidebar.
...
In this assignment we will use the following packages (you need to install them first):
